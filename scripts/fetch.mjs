...
if (!url) {
  console.error("node scripts/fetch.mjs <url> [--find a,b] [--raw] [--rung curl|jina|playwright]");
  process.exit(1);
}
...
async function viaPlaywright(target) {
  ...
  let status = 0;
  try {
    const res = await page.goto(target, { waitUntil: "networkidle", timeout: 90000 });
    status = res?.status() ?? 0;
  } catch {
  ...
  const readBody = async () =>
    blocks.push(await page.evaluate(() => (document.body.innerText ?? "").replace(/\s+/g, " ").trim()));
