...
export const ALDRIG_BEDOMD = [
  ...
  "Batteritid med GPS",
  /* Tillkom 2026-08-06 med /stavmixer. Hela sidans fynd är att watt mäter
     vägguttaget och varvtal mäter kniven, och att de två inte följer varandra:
     Philips anger 800 W och 11 500 v/min, Bamix 200 W och 17 000 till 18 000.
     Åtta av tolv tillverkare anger inget varvtal alls. Ett tal härlett ur watt,
     eller lånat från en systermodell, raderar precis den spridningen. Bamix
     Cordless går 8 000 och 13 000 medan Swissline går 12 000 och 18 000, alltså
     räcker det inte ens att låna inom märket. */
  "Varvtal",
  "Provad av Which?",
  "Angiven noggrannhet",
  "Noggrannhet fukt",
  ...
  "Cykler till 80 %",
  "Ljudnivå",
  /* Tillkom 2026-08-06 med /blender. Applitest GmbH tog tid på nio namngivna
     maskiner när de gjorde samma smoothie, och spannet är 45 till 147
     sekunder. Talet får aldrig härledas ur motoreffekten, eftersom sidans hela
     fynd är att effekten pekar åt fel håll: de två maskinerna på 1 200 W tog
     45 och 55 sekunder, de två på 1 800 W tog 90 och 147. En härledd tid hade
     alltså inte bara varit påhittad utan systematiskt fel. Fyra av tretton
     produkter ingick inte i provningen och ska synas sakna talet. */
  "Beredningstid smoothie",
];
 ...
 * ## Stavmixer
 *
 * Byggd 2026-08-06. Tredje sidan i gruppen Kök.
 *
 * ### Varför `Varvtal` står här trots att åtta av tolv saknar det
 *
 * Fältet är sidans hela fynd och samtidigt dess glesaste rad, och båda sakerna
 * är avsiktliga. Watt mäter motorns anslutningseffekt, alltså elen in i
 * vägguttaget. Varvtal mäter kniven, alltså det maten möter. De två följer inte
 * varandra: Philips anger 800 W och 11 500 v/min, Bamix 200 W och 17 000 till
 * 18 000. Fyra tillverkare i fältet anger ett varvtal — Bamix för två modeller,
 * Wilfa och (bara via Icecat) KitchenAid — och åtta anger inget alls.
 *
 * Raden är därför `alltid: false` och kriteriet finns inte. Att fylla cellen åt
 * de åtta som tiger raderar exakt den spridning sidan finns för att visa, och
 * ett kriterium på fyra av tolv hade låtit omfördelningen avgöra ordningen.
 * Samma beslut som `Batteritid med GPS` på /smartwatch och `Skumtemperatur` på
 * /mjolkskummare.
 *
 * ### `Effekt` är tillverkarens specfält, aldrig säljtextens tal
 *
 * Brauns egen svenska produktsida för MultiQuick 9 MQ 9135XI anger `Effekt (W)
 * 1000` i två specifikationsrutor och `1200 W` tre gånger i säljtexten på samma
 * sida. MQ7035X anger 850 W i specfältet och säljs av handeln som 1 000 W.
 * Butikerna återger marknadsföringstalet. Vi tar specfältet, av samma skäl som
 * JBL Charge 6 fick 24 timmar och inte 28 på /bluetooth-hogtalare.
 */
/**
 * ## Blender
 *
 * Systersida till `stavmixer`, och de två delar sitt centrala fynd: **watt
 * mäter vägguttaget och säger ingenting om vad som kommer ur kannan.** På
 * stavmixersidan visas det genom att varvtalet inte följer effekten. Här visas
 * det med uppmätta resultat, vilket är starkare.
 *
 * Applitest GmbH mätte nio blendrar åt Testfakta. Sorterat efter motoreffekt
 * blir beredningstiden för en smoothie 45 och 55 sekunder för de två på
 * 1 200 W, och 90 och 147 sekunder för de två på 1 800 W. Nöthacket lutar åt
 * samma håll, 9,5 och 7,9 mot 4,8 och 4,0. **De två svagaste motorerna i
 * fältet gör alltså de två snabbaste smoothiesarna och det bästa nöthacket.**
 *
 * `Motoreffekt` står ändå kvar som markerad rad, eftersom det är talet läsaren
 * kommer för att leta efter. Den bär ingen egen vikt i viktningen; köpguiden
 * förklarar varför den inte betyder det man tror. Samma konstruktion som
 * `Angiven maxtemperatur` på /pizzaugn.
 *
 * ### `Kannans volym` är arbetsvolymen, aldrig bräddvolymen
 *
 * Kategorins andra fynd, och tre tillverkare skriver ut båda talen själva:
 *
 *   Bosch MMB6652B   "XLarge 3,0-liter max / 2,0-liter vid användning"
 *   Philips HR3760   "2 l glaskanna med 1,8 l effektiv kapacitet"
 *   Ninja TB301EU    "tillbringare på 2,1 liter … 1,9 liter för flytande ämnen"
 *
 * Handeln trycker det större talet: Bagaren och Kocken säljer Braun JB9040
 * under rubriken "3 liter" där Testfakta deklarerar 2 000 ml. Raden bär
 * **arbetsvolymen** för alla, alltså det maskinen faktiskt mixar, eftersom en
 * tabell där hälften är bräddmått och hälften arbetsmått inte jämför något.
 * Bräddvolymen har en egen omarkerad rad där tillverkaren publicerar den.
 *
 * ### Vad som aldrig får lånas
 *
 * `Ljudnivå` och `Beredningstid smoothie` ligger i `ALDRIG_BEDOMD`. Båda är
 * uppmätta av ett labb på nio namngivna maskiner, och spridningen är hela
 * poängen: 83 dB för KitchenAid mot 94 för Braun, 45 sekunder mot 147. Ett tal
 * härlett ur effekt eller lånat från en systermodell raderar precis den
 * spridningen — och skulle dessutom peka åt fel håll, eftersom effekten inte
 * förutsäger någotdera. Fyra av tretton produkter i fältet ingick inte i
 * provningen och ska synas sakna talen.
 */
const BLENDER = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Talet handeln säljer på, och det som bär noll vikt. Se blockkommentaren. */
  F("Motoreffekt", {
    enhet: "W",
    alltid: true,
    alias: ["Effekt", "Watt", "Anslutningseffekt", "Watt, max", "Ineffekt", "Märkeffekt"],
    monster: /(\d[\d\s]*)\s*(?:W|watt)\b/i,
  }),
  /* Arbetsvolym, aldrig bräddvolym. Se blockkommentaren. */
  F("Kannans volym", {
    enhet: "ml",
    alltid: true,
    alias: ["Volym kannan", "Kapacitet", "Effektiv kapacitet", "Arbetsvolym", "Kannvolym"],
    monster: /(\d+[,.]?\d*)\s*(?:liter|l)\b/i,
  }),
  F("Material kannan", {
    alltid: true,
    alias: ["Kannans material", "Material", "Kannmaterial", "Jar material"],
  }),
  /* Uppmätt av Applitest. Får aldrig lånas — se ALDRIG_BEDOMD. */
  F("Ljudnivå", {
    enhet: "dB",
    alias: ["Bullernivå", "Ljudnivå min - max", "Decibel", "Noise level"],
    monster: /(\d+[,.]?\d*)\s*dB/i,
  }),
  /* Uppmätt av Applitest. Får aldrig lånas — se ALDRIG_BEDOMD. */
  F("Beredningstid smoothie", {
    enhet: "sek",
    alias: ["Beredningstid", "Tid smoothie", "Smoothietid"],
    monster: /(\d+)\s*(?:sek|s)\b/i,
  }),
  /* Tyngsta fältet i kriteriet Kontroll över mixningen. Spannet går från
     steglöst med tolv steg till ingen hastighetsinställning alls: Chef Matteo
     Blender III har enbart fasta program. */
  F("Hastighetsinställning", {
    alltid: true,
    alias: ["Hastigheter", "Antal hastigheter", "Hastighetsreglering", "Hastighetsinställningar", "Speed settings"],
  }),
  F("Diskmaskinsäker kanna", {
    alltid: true,
    alias: ["Diskmaskinsäker", "Tål maskindisk", "Maskindisk", "Dishwasher safe"],
  }),
  /* Ett åtagande säljaren gör och alltså ett villkor för köpet. Spannet är
     faktor fem: Bosch anger 10 års motorgaranti mot registrering, Wilfa 5 år
     på hela produkten, och resten av fältet 2 år. */
  F("Garanti", {
    enhet: "år",
    alias: ["Garantitid", "Motorgaranti", "Warranty"],
    monster: /(\d+)\s*års?\s*(?:motor)?garanti/i,
  }),
  /* Omarkerade nedan: mätta av Applitest på nio, alltså för glesa för
     jämförelsetabellen men värdefulla i produktens egen specifikationslista.
     Se varningen i check-tackning om att inte radera dem. */
  F("Kannans bräddvolym", {
    enhet: "ml",
    alias: ["Bräddvolym", "Maxvolym", "Total volym", "Kannans totala volym"],
  }),
  F("Säkerhetslock", {
    alias: ["Säkerhetslock till kannan", "Säkerhetsstopp", "Lockspärr"],
  }),
  F("Totalvikt", {
    enhet: "g",
    alias: ["Vikt", "Nettovikt", "Produktvikt"],
    monster: /(\d+[,.]?\d*)\s*(?:kg|g)\b/i,
  }),
  F("Kannans vikt med lock", {
    enhet: "g",
    alias: ["Kannans vikt", "Kannvikt"],
  }),
  F("Antal program", {
    alias: ["Program", "Förinställda program", "Automatiska program", "Programs"],
    monster: /(\d+)\s*(?:förinställda\s*)?program/i,
  }),
  F("Varvtal", {
    enhet: "v/min",
    alias: ["RPM", "Rotationshastighet", "Varv per minut"],
    monster: /([\d\s'.,]+)\s*(?:v\/min|varv|rpm)/i,
  }),
  F("GTIN"),
];
...
const STAVMIXER = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Talet hela handeln säljer på, och det som väger minst i viktningen.
     Tillverkarens specfält gäller, aldrig säljtextens. */
  F("Effekt", {
    enhet: "W",
    alltid: true,
    alias: ["Watt", "Anslutningseffekt", "Motoreffekt", "Märkeffekt", "Wattage"],
    monster: /(\d+)\s*(?:W|watt)\b/i,
  }),
  /* Sidans fynd. Se blockkommentaren ovan för varför raden får vara gles. */
  F("Varvtal", {
    enhet: "v/min",
    alias: ["RPM", "Rotationshastighet", "Varv/min", "Varvtal max", "Speed"],
    monster: /([\d\s'.,–-]+)\s*(?:v\/min|varv|rpm)/i,
  }),
  /* Tyngsta kriteriet. Spannet går från en enda fast fart till steglöst med
     turbo, och det avgör om majonnäsen går att starta långsamt. */
  F("Hastighetsreglering", {
    alltid: true,
    alias: ["Hastigheter", "Antal hastigheter", "Hastighetsinställningar", "Speed settings"],
  }),
  F("Mixerfotens material", {
    alltid: true,
    alias: ["Material mixerfot", "Mixerfot", "Skaftmaterial", "Material skaft"],
  }),
  F("Antal knivblad", {
    alias: ["Antal knivar", "Knivblad", "Blad", "Number of blades"],
    monster: /(\d+)\s*(?:blad|knivar|-vingat)/i,
  }),
  F("Löstagbar mixerfot", {
    alias: ["Löstagbar fot", "Avtagbart skaft", "Detachable shaft"],
  }),
  F("Tillbehör", {
    alltid: true,
    alias: ["Medföljande tillbehör", "Ingår", "Vad finns i boxen", "Accessories"],
  }),
  F("Bägarens volym", {
    enhet: "ml",
    alias: ["Bägare", "Mixerbägare", "Skålkapacitet", "Behållare"],
  }),
  /* Ett åtagande säljaren gör, alltså ett villkor för köpet och inte en uppgift
     i ett produktblad. OBH håller reservdelar i 15 år, Bamix ger livstids
     motorgaranti, Ninja anger 1 år. */
  F("Reservdelsåtagande", {
    alias: ["Reservdelar", "Reparerbarhet", "Reparerbarhetsgaranti"],
  }),
  F("Garanti", {
    enhet: "år",
    alltid: true,
    alias: ["Garantitid", "Warranty"],
    monster: /(\d+)\s*års?\s*garanti/i,
  }),
  F("Strömkälla", { alias: ["Drivning", "Sladd eller batteri", "Power source"] }),
  F("Sladdlängd", {
    enhet: "m",
    alias: ["Kabellängd", "Kabel längd", "Nätsladdens längd"],
    monster: /(\d+[,.]?\d*)\s*m\b/i,
  }),
  F("Vikt", {
    enhet: "kg",
    alias: ["Nettovikt", "Netto vikt", "Produktvikt", "Weight"],
    monster: /(\d+[,.]?\d*)\s*kg\b/i,
  }),
  F("Maskindiskbara delar", {
    alias: ["Tål maskindisk", "Diskmaskinssäker", "Dishwasher-safe"],
  }),
  F("Artikelnummer"),
  F("GTIN"),
];
 ...
 * ## Smoothiemixer
 *
 * Personliga mixrar där smoothien blandas direkt i muggen du sedan dricker ur.
 * Bänkblendern på 1,4 till 2 liter är en annan produkt och har sitt eget
 * schema, `BLENDER`.
 *
 * ### `Mixningar per laddning` är hyllans tal, inte köparens
 *
 * Fältet bär det tillverkaren skriver på kartongen, och det är avsiktligt, för
 * det är talet läsaren möter i butiken. Det jämför bara inte det man tror:
 *
 *   Ninja Blast Max      25 mixningar   à 30 sekunder   = 12 min 30 s
 *   KitchenAid Go        20 minuter     à 60 sekunder   = 20 min
 *   Wilfa Swift          14 mixningar   à 35 sekunder   = 8 min 10 s
 *   Ninja Blast          10 mixningar   à 30 sekunder   = 5 min
 *
 * Cykellängden står i manualen och aldrig på produktsidan. Den omräknade
 * mixtiden ligger i `Mixtid per laddning`, omarkerad, eftersom den bara går
 * att räkna fram för de fyra ovan.
 *
 * De nätdrivna bär värdet `Nätdriven` i raden. Det är rätt svar och inte en
 * lucka: en mixer med sladd tar inte slut.
 *
 * ### `Motoreffekt` är markerad trots att fem av elva saknar den
 * 6/11 anger watt. De sladdlösa anger mAh eller volt i samma fält —
 * Nutribullet skriver `Effekt: 2000mAh Battery` i sin egen tabell. Raden står
 * kvar markerad därför att det är talet läsaren letar efter, och `Kapacitet`
 * plus `Mixningar per laddning` bär jämförelsen där watten saknas. Samma
 * konstruktion som `Angiven maxtemperatur` på /pizzaugn.
 *
 * ### `Kapacitet` är max fyllnadsvolym, aldrig kartongens tal
 *
 * Två tillverkare skriver ut båda själva: Ninja Blast Max `570 ml (max
 * fyllning 490 ml)` och Ninja BlendBoss `710 ml (max 650 ml fyllnad)`.
 * Marknadsföringen trycker det större. Raden bär det mindre, alltså det du
 * faktiskt får blanda, eftersom en tabell med hälften bräddmått inte jämför
 * något. Samma beslut som `Kannans volym` i `BLENDER`.
 *
 * ### `Max körtid per gång` är omarkerad och ändå sidans skarpaste uppgift
 *
 * Den står bara i manualerna. OBH Nordica 7740: "Maximal användningstid:
 * 1 minut, vänta minst 5 minuter innan apparaten används igen." Smeg: 60
 * sekunder åt gången med 60 sekunders paus. Två nätdrivna mixrar på samma
 * hylla, och den ena låter dig göra nästa smoothie fem gånger snabbare.
 * Omarkerad eftersom bara en del av fältet publicerar den; talet bär i stället
 * ett eget avsnitt på sidan, som drifttiden vid full effekt på
 * /skaftdammsugare.
 */
const SMOOTHIEMIXER = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Batteri eller sladd. Kategorins avgörande konstruktionsfält. */
  F("Drivsätt", {
    alltid: true,
    alias: ["Strömkälla", "Drift", "Strömförsörjning", "Power source"],
  }),
  /* Markerad trots 6/11. Se blockkommentaren ovan. */
  F("Motoreffekt", {
    enhet: "W",
    alltid: true,
    alias: ["Effekt", "Watt", "Watt, max", "Intensitet", "Märkeffekt"],
    monster: /(\d[\d\s]*)\s*(?:W|watt)\b/i,
  }),
  /* Max fyllnadsvolym, aldrig bräddvolym. Se blockkommentaren ovan. */
  F("Kapacitet", {
    enhet: "ml",
    alltid: true,
    alias: ["Volym", "Muggens volym", "Kapacitet (vätskor)", "Skålkapacitet", "Max fyllnadsvolym"],
    monster: /(\d+[,.]?\d*)\s*(?:ml|liter|l)\b/i,
  }),
  /* Hyllans tal. `Nätdriven` är ett riktigt värde här. */
  F("Mixningar per laddning", {
    alltid: true,
    alias: ["Antal mixningar", "Blandningscykler", "Cykler per laddning", "Blends per charge"],
  }),
  F("Antal muggar", {
    alltid: true,
    alias: ["Muggar", "Antal bägare", "Bägare", "Antal flaskor"],
  }),
  F("Maskindiskbara delar", {
    alltid: true,
    alias: ["Diskmaskinsäker", "Tål maskindisk", "Maskindisk", "Diskvattensäkra delar", "Delar att diska"],
  }),
  /* Omräknad ur mixningar gånger cykellängd. Bara där båda talen finns. */
  F("Mixtid per laddning", {
    alias: ["Drifttid", "Total mixtid", "Blending time"],
  }),
  F("Programcykel", {
    enhet: "sek",
    alias: ["Cykellängd", "Programlängd", "Blendcykel"],
    monster: /(\d+)\s*(?:sek|s)\b/i,
  }),
  /* Står bara i manualerna. Se blockkommentaren ovan. */
  F("Max körtid per gång", {
    alias: ["Maximal användningstid", "Max drifttid", "Kontinuerlig drifttid"],
  }),
  F("Batterispänning", {
    enhet: "V",
    alias: ["Spänning batteri", "Battery voltage"],
    monster: /(\d+[,.]?\d*)\s*V\b/,
  }),
  F("Laddningstid", {
    enhet: "h",
    alias: ["Laddtid", "Charging time"],
    monster: /(\d+[,.]?\d*)\s*(?:h|tim)/i,
  }),
  F("Varvtal", {
    enhet: "rpm",
    alias: ["Rotationer per minut", "Rotationshastighet", "RPM"],
    monster: /(\d[\d\s]*)\s*(?:rpm|varv)/i,
  }),
  F("Material mugg", {
    alias: ["Muggens material", "Material", "Blandningsskålens material", "Flaskans material"],
  }),
  F("Vikt", {
    enhet: "kg",
    alias: ["Nettovikt", "Produktvikt"],
  ...
  F("Yttermått", { alias: ["Mått", "Produktmått", "Dimensioner"] }),
  F("Sladdlängd", { enhet: "m", alias: ["Kabellängd", "Sladd"] }),
  F("Garanti", { enhet: "år", alias: ["Garantitid"], monster: /(\d+)\s*års?\s*garanti/i }),
  F("GTIN"),
];
 ...
 * ## Elscooter
 *
 * Kategorins hela poäng ligger i att **två effekttal** måste stå bredvid
 * varandra. Transportstyrelsen klassar en elsparkcykel som cykel bara om den
 * klarar både 20 km/h och 250 W *kontinuerlig märkeffekt*. Det är det
 * nominella talet lagen mäter, medan handeln annonserar toppeffekten.
 *
 * `Nominell effekt` är därför en grind som varenda rankad produkt passerar —
 * alla fem anger 250 W, eftersom det är taket — och `Toppeffekt` är axeln som
 * faktiskt rangordnar dem, 450 till 1 200 W. Båda står i tabellen ändå: en
 * tabell med bara toppeffekten hade dolt varför sidan finns, och en med bara
 * den nominella hade visat en kolumn med fem likadana tal. Se
 * `.agent/research/elscooter.md` §4g och §4j.
 *
 * `Räckvidd` bär tillverkarens eget tal och är medvetet **inte** betygsatt.
 * E-Wheels publicerar två tal per modell — 45 km "optimal" mot 23–27 km
 * "förväntad" — Segway anger 25 km vid 15 km/h och 20 km vid 20 km/h, och
 * Pure mäter i ekoläge. Talen är inte jämförbara med varandra, så betyget för
 * batteri sätts på wattimmar, som är hård varufakta och finns för alla fem.
 * Samma skäl som `Angiven besparing` på /smart-termostat.
 */
const ELSCOOTER = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Lagens tal. Grind, inte axel — se blockkommentaren ovan. */
  F("Nominell effekt", {
    enhet: "W",
    alltid: true,
    alias: [
    ...
    monster: /(\d{3,4})\s*W\s*(?:nominell|märkeffekt|rated)/i,
  }),
  /* Talet som rangordnar. Handeln kallar det omväxlande topp- och maxeffekt. */
  F("Toppeffekt", {
    enhet: "W",
    alltid: true,
    alias: ["Maxeffekt", "Max effekt", "Max power", "Peak power"],
    monster: /(\d{3,4})\s*W\s*(?:max|topp|peak)/i,
  }),
  F("Topphastighet", {
    enhet: "km/h",
    alias: ["Maxhastighet", "Max hastighet", "Max speed"],
    monster: /(\d{2})\s*km\/(?:h|tim)/i,
  }),
  F("Batteri", {
    enhet: "Wh",
    alltid: true,
    alias: ["Batterikapacitet", "Battery", "Nominal Energy", "Batteristorlek"],
    monster: /(\d{3,4})\s*Wh/i,
  }),
  F("Räckvidd", {
    alias: ["Optimal räckvidd", "Max räckvidd", "Range", "Räckvidd fulladdad"],
  }),
  F("Vikt", {
    enhet: "kg",
    alltid: true,
    alias: ["Nettovikt", "Net weight", "Produktvikt", "Egenvikt"],
    monster: /(\d+[,.]?\d*)\s*kg/i,
  }),
  F("Bromsar", { alias: ["Bromssystem", "Broms", "Brakes", "Bromstyp"] }),
  F("Däck", { alias: ["Däcktyp", "Hjul", "Hjulstorlek", "Tyres", "Däckstorlek"] }),
  F("Dämpning", {
    alias: ["Dämparsystem", "Fjädring", "Stötdämpare", "Suspension"],
  }),
  /* Omärkta med flit: de bor i produktens egen speclista. Max lutning anges av
     två av fem, laddtiden beror på vilken laddare som följer med, och hopfällt
     mått är intressant först när valet redan är gjort. Rader av dem hade gett
     tre kolumner med streck. */
  F("Kapslingsklass", {
    alias: ["IP-grad", "IP-klass", "Vattentålighet", "Water resistance"],
    monster: /IP\s?X?\d{1,2}/i,
  }),
  F("Max förarvikt", {
    enhet: "kg",
    alias: ["Max belastning", "Maxlast", "Vikt Förare", "Max load"],
    monster: /(\d{2,3})\s*kg/i,
  }),
  F("Max lutning", {
    enhet: "%",
    alias: ["Klättringsförmåga", "Max backlutning", "Climbing gradient"],
    monster: /(\d{1,2})\s*%/i,
  }),
  F("Laddtid", { enhet: "h", alias: ["Laddningstid", "Charging time"] }),
  F("Mått hopfälld", {
    alias: ["Storlek hopfälld", "Hopfällt mått", "Folded dimensions"],
  }),
  F("GTIN"),
];
 ...
 * ## Airfryer
 *
 * Fälten följer sidans fem kriterier, beslutade 2026-08-06, och det tal hela
 * sidan finns för: **hur många kammare maskinen har och hur mycket effekt var
 * och en får**.
 *
 * `Kammare` är kategorins avgörande konstruktionsfält, av samma slag som
 * `Stenens rörelse` på /pizzaugn. Tre värden förekommer: en enda låda, två
 * fasta lådor, och en låda med löstagbar delare som går att köra som en enda
 * stor kammare. Skillnaden mellan den andra och den tredje är hela skälet att
 * sidan finns.
 *
 * `Effekt per kammare` bär `harledd`: märkeffekten delad med antalet kammare
 * som körs samtidigt. Räkningen är RTINGS, som mätt Ninja Foodi DZ201 till
 * 1 540 W med båda lådorna igång och 1 470 W med bara en — den andra lådan
 * lägger alltså till 70 watt. Ninjas egna bruksanvisningar anger 2 470 W för
 * både AF300EU och AF400EU, alltså totalt för två zoner.
 *
 * `Korgens bottenyta` är RTINGS tröskelvärde, 325 cm², och det tal som
 * korrelerar starkast med andelen krispiga pommes i deras provning. Det är
 * **inte markerat**, eftersom bara AIVIQ publicerar korgmått i svensk handel.
 * Ett omarkerat fält på tre av tio produkter är sourcad data i produktens egen
 * lista, inte en trasig jämförelserad — se varningen i `check:tackning`.
 */
const AIRFRYER = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Kategorins avgörande konstruktionsfält. Se blockkommentaren ovan. */
  F("Kammare", {
    alias: ["Antal korgar", "Antal lådor", "Zoner", "Enkel eller dubbel korg", "Baskets"],
  }),
  F("Total volym", {
    enhet: "l",
    alias: ["Kapacitet", "Volym", "Kapacitet (l)", "Capacity"],
    monster: /(\d+[,.]?\d*)\s*(?:l\b|liter)/i,
  }),
  F("Volym per kammare", {
    enhet: "l",
    alias: ["Volym per låda", "Kapacitet per korg", "Zonvolym"],
  }),
  F("Effekt", {
    enhet: "W",
    alias: ["Märkeffekt", "Anslutningseffekt", "Watt", "Effekt (watt)", "Power"],
    monster: /(\d[\d\s]{2,5})\s*W\b/i,
  }),
  /* Härledd ur märkeffekt och antal kammare. Aldrig lånad från en systermodell. */
  F("Effekt per kammare", { enhet: "W", alias: ["Effekt per låda", "Effekt per zon"] }),
  F("Maxtemperatur", {
    enhet: "°C",
    alias: ["Max temperatur", "Högsta temperatur", "Maximal temperatur"],
    monster: /(\d+)\s*°?\s*C/i,
  }),
  F("Temperaturomfång", {
    alias: ["Temperaturintervall", "Temperaturområde", "Temperature range"],
  }),
  F("Angiven matmängd", {
    enhet: "kg",
    alias: ["Fritörkapacitet", "Kapacitet (kg)", "Total kapacitet (kg)", "Matmängd"],
  }),
  F("Beläggning", {
    alias: ["Ytbeläggning", "Nonstick-beläggning", "Material korg", "Coating"],
  }),
  F("Delar att diska", { alias: ["Löstagbara delar", "Diskbara delar"] }),
  F("Antal program", {
    alias: ["Program", "Förinställda program", "Antal förinställda program", "Presets"],
  }),
  F("Yttermått", { alias: ["Mått", "Produktmått", "Produktstorlek", "Dimensioner"] }),
  F("Vikt", {
    enhet: "kg",
    alias: ["Nettovikt", "Produktvikt", "Netto vikt"],
    monster: /(\d+[,.]?\d*)\s*kg\b/i,
  }),
  /* Omarkerat med flit. Se blockkommentaren ovan. */
  F("Korgens bottenyta", { enhet: "cm²", alias: ["Bottenyta", "Korgmått", "Basket surface area"] }),
  F("Garanti", { enhet: "år", alias: ["Garantitid"], monster: /(\d+)\s*års?\s*garanti/i }),
  F("GTIN"),
];
 ...
 * ## Fritös
 *
 * Oljefritös, alltså den klassiska frityrkokaren. Varmluft ligger på
 * `/airfryer` och delar inget fältschema med den här: en airfryer har ingen
 * oljemängd att ange, och det är just oljemängden hela sidan handlar om.
 *
 * `Oljemängd` och `Matmängd` är kategorins två avgörande tal, och poängen är
 * att de **inte följer varandra**. Tefal Easy Pro, Princess 182727 och Severin
 * FR 2431 tar alla tre 3,0 liter olja och friterar 1,2 kg, 0,6 kg respektive
 * 0,4 kg. Handeln anger genomgående literangivelsen och sällan gramtalet.
 *
 * `Oljeåtgång` bär `harledd`: oljemängden delad med matmängden. Talet är inte
 * hämtat någonstans utan räknat ur två tal tillverkaren själv publicerar, och
 * det spänner från 1,54 till 7,50 l/kg över fältet.
 *
 * ⚠️ **Matmängden är tillverkarens egen friteringskapacitet, och alla menar
 * inte samma sak med den.** Tefal anger för FR7016 både `Livsmedelskapacitet
 * 800 g` och `Pommes frites-kapacitet 600 g` på samma sida. Där båda finns
 * används livsmedelskapaciteten, eftersom det är den handeln citerar. Det står
 * i metodrutan.
 *
 * `Kallzon` är **omarkerat med flit.** Ett område i botten som är svalare, dit
 * smulor sjunker utan att brännas. Åtta av elva anger den, tre gör det inte —
 * och de tre är inte produkter utan kallzon, utan produkter där uppgiften inte
 * gått att belägga hos tillverkaren. En markerad rad hade gjort vår research
 * till ett betyg. Se `check:tackning` och `check:avdrag`.
 *
 * `Oljefilter` är däremot markerat och bär vikt, eftersom det separerar fältet
 * på riktigt: två maskiner silar och lagrar oljan automatiskt, fyra har ett
 * fast filter, fem har inget.
 */
const FRITOS = [
  F("Pris", { enhet: "kr", alltid: true }),
  F("Oljemängd", {
    enhet: "l",
    alltid: true,
    alias: ["Oljekapacitet", "Oljeinnehåll", "Kapacitet", "Capacity(l)", "Oil capacity", "Volym olja"],
    monster: /(\d+[,.]?\d*)\s*(?:l\b|liter)/i,
  }),
  F("Matmängd", {
    enhet: "g",
    alltid: true,
    alias: ["Livsmedelskapacitet", "Fritörkapacitet", "Friteringskapacitet", "Capacity(g)", "Capacity(kg)", "Frying capacity", "Matkapacitet"],
    monster: /(\d+[,.]?\d*)\s*(?:g|gram|kg)\b/i,
  }),
  /* Härledd ur de två raderna ovan. Aldrig hämtad, aldrig lånad från en
     systermodell. Se blockkommentaren. */
  F("Oljeåtgång", { enhet: "l/kg", alias: ["Olja per kilo", "Oljeåtgång per kilo mat"] }),
  F("Effekt", {
    enhet: "W",
    alltid: true,
    alias: ["Märkeffekt", "Watt", "Wattage", "Power", "Effekt (W)"],
    monster: /(\d[\d\s]{2,5})\s*W\b/i,
  }),
  F("Oljefilter", {
    alias: ["Filtrering", "Oljefiltrering", "Clean & safety oil filter", "Filter"],
  }),
  F("Maxtemperatur", {
    enhet: "°C",
    alias: ["Max temperatur", "Maximum temperature", "Temperatur (max)", "Högsta temperatur"],
    monster: /(\d+)\s*°?\s*C/i,
  }),
  F("Löstagbar oljebehållare", {
    alias: ["Flyttbar skål", "Avtagbar skål", "Removable bowl", "Löstagbar skål"],
  }),
  F("Delar som tål maskindisk", {
    alias: ["Diskvattensäkra delar", "Dishwasher proof parts", "Diskbara delar", "Tål maskindisk"],
  }),
  F("Beläggning", {
    alias: ["Ytbeläggning", "Invändigt ytskikt", "Skålmaterial", "Non-stick coating", "Coating"],
  }),
  /* Omarkerat med flit. Se blockkommentaren ovan. */
  F("Kallzon", { alias: ["Kalla zonen", "Cool zone", "Cold zone", "Kaltzone"] }),
  F("Löstagbart värmeelement", { alias: ["Borttagbart element", "Removable heating element"] }),
  F("Sladdlängd", { enhet: "m", alias: ["Power cord length", "Kabellängd"] }),
  F("Yttermått", { alias: ["Mått", "Produktmått", "Dimensioner", "Dimensions of the product"] }),
  F("Vikt", {
    enhet: "kg",
    alias: ["Nettovikt", "Net weight", "Produktvikt"],
    monster: /(\d+[,.]?\d*)\s*kg\b/i,
  }),
  F("GTIN"),
];
 ...
 *
 * Magnetkontakten som rapporterar öppet eller stängt till en app eller en
 * hubb. Det fristående sirenlarmet som sitter på fönstret och tjuter är en
 * annan produkt med eget schema, `FONSTERLARM`.
 *
 * ### `Kräver hubb` är kategorins avgörande fält
 *
 * Fältet bär vad sensorn måste prata med, och det delar fältet i två läger som
 * ingen svensk konkurrent skriver ut. Åtta produkter talar en öppen standard
 * och fungerar med vilken hubb som helst i den standarden; fyra är låsta till
 * ett enda märkes basstation:
 *
 *   TP-Link Tapo T110    Tapo H100 eller H200
 *   Ring Alarm Contact   Ring Alarm-basstation
 *   eufy Security        eufy HomeBase eller HomeBase E
 *   Philips Hue Secure   Philips Hue Bridge
 *   Yale Magnetkontakt   Yale Smart Hub
 *
 * Följden är att sensorns pris inte är priset. Billigaste sensorn i fältet,
 * Tapo T110 på 129 kronor, kräver en hubb som kostar mer än den själv.
 *
 * ### `Mått sensor` är markerad trots att fem av fjorton saknar den
 *
 * Raden ligger på 9/14 och alltså över tröskeln i `check:tackning`. Den står
 * markerad därför att det är den uppgift som avgör om sensorn får plats på en
 * smal fönsterkarm, och därför att **ingen svensk konkurrent anger den alls**.
 * Spannet är en faktor tjugofyra i volym:
 *
 *   Shelly BLU Door/Window ZB    35 × 35 × 7      8 600 mm³
 *   Aqara Sensor P2              77 × 22 × 22    37 000 mm³
 *   Yale utomhus                 94 × 63,5 × 35  209 000 mm³
 *
 * Måtten kom ur manualerna och tillverkarnas specifikationsflikar, inte ur
 * butikernas specblock, som svarade tomt på samtliga. Cleverios 54 × 22 × 12
 * står i den PDF Kjell länkar från produktsidan, Aqara P2:s 77 × 22 × 22 på
 * `aqara.com/en/…/specs/` — medan `/eu/`-sökvägen svarar 404.
 *
 * ### `Sabotageskydd` bär ingen vikt, och det är avsiktligt
 *
 * Sex av fjorton har det belagt: Cleverio och Fibaro ur manualen, Yale inne
 * och ute ur Kjells specblock, Sonoff ur Proshops beskrivning och Aqara P2 ur
 * tillverkarens egen text. De övriga åtta **nämner det inte**, vilket är vår
 * research och inte produktens egenskap. Ett kriterium hade därför betygsatt
 * vem som skrivit ned uppgiften. Raden ligger i tabellen och bär noll vikt,
 * samma konstruktion som `Varvtal` på /stavmixer.
 *
 * ### `Batteritid` och `Mått sensor` är en avvägning, utom hos en
 *
 * Sonoff och ThirdReality köper sin drifttid med två AAA-celler och blir stora
 * av det. Shelly når samma tre år på ett enda CR2032 i fältets minsta hölje.
 * Cleverio ligger lägst på ca 1 år, alltså fyra batteribyten där Yale klarar
 * sig på ett.
 */
const DORR_OCH_FONSTERSENSOR = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Kategorins avgörande fält. Se blockkommentaren ovan. */
  F("Kräver hubb", {
    alltid: true,
    alias: ["Hubbkrav", "Kräver gateway", "Gateway", "Basstation", "Kräver bas", "Hub required"],
  }),
  F("Protokoll", {
    alltid: true,
    alias: ["Trådlös standard", "Anslutning", "Radio", "Kommunikation", "Trådlöst", "Wireless protocol"],
  }),
  F("Batterityp", {
    alltid: true,
    alias: ["Batteri", "Battery", "Batterier", "Strömförsörjning"],
  }),
  F("Batteritid", {
    enhet: "år",
    alltid: true,
    alias: ["Batterilivslängd", "Drifttid", "Battery life", "Standbytid", "Standby-tid"],
    monster: /(\d+[,.]?\d*)\s*(?:år|years?)\b/i,
  }),
  /* Markerad trots 9/14. Se blockkommentaren ovan. */
  F("Mått sensor", {
    enhet: "mm",
    alltid: true,
    alias: ["Mått", "Storlek", "Storlek (sensor)", "Dimensioner", "Yttermått", "Size", "Dimensions"],
    monster: /(\d+[,.]?\d*)\s*[x×]\s*(\d+[,.]?\d*)\s*[x×]\s*(\d+[,.]?\d*)\s*mm/i,
  }),
  /* Omarkerad. Magneten är den halva som ska få plats på den rörliga delen,
     och den är nästan alltid mindre än sensorn, men bara hälften anger den. */
  F("Mått magnet", {
    enhet: "mm",
    alias: ["Storlek (magnet)", "Magnetens mått", "Accessory unit", "Magnet size"],
  }),
  /* Ingen vikt. Se blockkommentaren ovan. */
  F("Sabotageskydd", {
    alias: ["Sabotagelarm", "Anti-tamper", "Tamper", "Tamperskydd", "Sabotageavkänning", "Manipuleringsdetektering"],
  }),
  F("Mäter mer än öppet/stängt", {
    alias: ["Extra sensorer", "Ytterligare mätvärden", "Fler funktioner", "Övriga sensorer"],
  }),
  F("Max glapp", {
    enhet: "mm",
    alias: ["Maxavstånd", "Detekteringsavstånd", "Avstånd sensor magnet", "Gap"],
  }),
  F("Montering", {
    alias: ["Fästmetod", "Infästning", "Monteringssätt", "Mounting"],
  }),
  F("Räckvidd", {
    enhet: "m",
    alias: ["Radioräckvidd", "Signalräckvidd", "Range"],
  }),
  F("Utomhusklassad", {
    alias: ["IP-klass", "IP-klassning", "Utomhusbruk", "Kapslingsklass", "Ingress protection"],
  }),
  F("Vikt", {
    enhet: "g",
    alias: ["Nettovikt", "Produktvikt", "Weight"],
 ...
 * ## Espressomaskin
 *
 * Helautomater, alltså bönmaskiner med inbyggd kvarn. Portafiltermaskinen är
 * en annan produkt för en annan köpare och har inget schema här; se
 * användarbeslutet i `.agent/research/espressomaskin.md` §8.
 *
 * ### `Mjölksystem` bär sidans tyngsta kriterium
 *
 * Fältet har fyra verkliga värden och de fördelar sig jämnt över handeln, till
 * skillnad från nästan allt annat i kategorin:
 *
 *   Manuell ångstav            du skummar själv i en kanna
 *   Slang till mjölkpaket      slangen ner i paketet, inget att diska
 *   Integrerad mjölkbehållare  kanna som sitter på maskinen
 *   Automatisk skummare        skummar utan kanna, sällan med ett tryck
 *
 * Raden står tillsammans med `Mjölkdrycker med ett tryck`, eftersom systemet
 * ensamt inte säger om du faktiskt får en cappuccino utan att flytta koppen.
 * Nivona CafeRomatica NICR 550 har automatisk skummare och **noll**
 * mjölkdrycker med ett tryck; Krups Evidence ECO har slangsystem och två.
 *
 * ### Två fält är grindar och finns därför inte som markerade rader
 *
 * `Portionsval för svart kaffe` anger **2 hos 52 av 54** maskiner i handeln,
 * och `Antal bönbehållare` anger **1 hos 42 av 54**. Ett kriterium på något av
 * dem hade delat ut vikt för en egenskap alla har. Samma beslut som
 * maxtemperaturen på /pizzaugn och `Larm när förbindelsen bryts` på /babyvakt.
 *
 * Antalet bönbehållare ligger kvar som **omarkerad** rad, eftersom de fyra i
 * handeln som har två kammare skiljer sig på riktigt.
 *
 * ### ⚠️ Butikens strukturerade attribut hade fel om just den raden
 *
 ...
 * kammare): Ja` och beskriver funktionen i prosa. **Tillverkaren gäller.**
 * Samma PIM anger `Intensitet: 230 W` för Philips EP5547/90, alltså
 * nätspänningen i ett effektfält. Butiksattribut är tier B och ska kontrolleras
 * mot tillverkaren för allt som bär vikt.
 *
 * ### `Kaffetemperatur, steg` bär ingen vikt med flit
 *
 * Råd & Rön har mätt kaffet ur 57 helautomater till mellan 53 och 71 grader och
 * skriver själva att "det går inte att se ett samband mellan till exempel en
 * låg temperatur och mindre gott kaffe". Ett kriterium på temperatursteg hade
 * viktat något den enda part som mätt saken säger inte spelar roll. Raden finns
 * omarkerad; talet bär köpguiden.
 */
const ESPRESSOMASKIN = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Kategorins avgörande konstruktionsfält. Se blockkommentaren ovan. */
  F("Mjölksystem", {
    alltid: true,
    alias: ["Mjölksystemtyp", "Typ av mjölksystem", "Mjölkskummare", "Milk system"],
  }),
  F("Mjölkdrycker med ett tryck", {
    alltid: true,
    alias: [
      "Portionsval för kaffe med mjölk",
      "Mjölkdrycker",
      "Antal mjölkdrycker",
      "One touch-drycker",
    ],
  ...
  F("Malningsgrader", {
    alltid: true,
    alias: [
      "Justerbar malning grovhet (nivåer)",
      "Antal kvarninställningar",
      "Malningsnivåer",
      "Inställning av malningsnivå",
      "Malningsgrad",
    ],
  }),
  F("Kvarntyp", { alltid: true, alias: ["Typ av kvarn", "Kvarnmaterial", "Kvarn"] }),
  /* Avtagbar eller fast. Avgör om bryggenheten går att skölja under kranen
     eller bara kan rengöras med tabletter genom maskinens eget program. */
  F("Bryggenhet", {
    alltid: true,
    alias: ["Kaffebryggningsenhet", "Bryggrupp", "Avtagbar bryggenhet", "Bryggningsenhet"],
  }),
  F("Kaffedrycker i menyn", {
    alltid: true,
    alias: ["Användarskapade recept", "Antal program", "Antal drycker", "Recept", "Drycker"],
  }),
  F("Sparade profiler", {
    alltid: true,
    alias: ["Sparade användarprofiler", "Antal användarprofiler", "Användarprofiler", "Profiler"],
  }),
  F("Vattentank", {
    enhet: "l",
    alltid: true,
    alias: ["Volym vattentank", "Vattentankens kapacitet", "Vattenbehållare"],
  }),
  F("Bönbehållare", {
    enhet: "g",
    alltid: true,
    alias: [
      "Kaffebönskapacitet",
      "Bönbehållarkapacitet",
      "Kapacitet hos behållare för färska kaffebönor",
    ],
  }),
  /* Omarkerad grind, men de som har två kammare skiljer sig på riktigt. */
  F("Antal bönbehållare", { alias: ["Antal behållare med bönor", "Bönfack"] }),
  F("Kaffestyrka, steg", {
    alias: ["Kaffestyrka (nivåer)", "Antal inställningar för kaffestyrka", "Styrkelägen"],
  }),
  /* Omarkerad med flit. Se blockkommentaren ovan. */
  F("Kaffetemperatur, steg", {
    alias: ["Kaffetemperatur (nivåer)", "Antal temperaturinställningar", "Temperaturlägen"],
  }),
  F("Mjölkbehållare", { enhet: "l", alias: ["Mjölktankens kapacitet", "Mjölkkanna"] }),
  F("Rengöringsprogram mjölksystem", {
    alias: [
      "Mjölk system rengörings program",
      "Rengöringsprogram för mjölksystem",
      "autoMilk Clean",
    ],
  }),
  F("Automatisk sköljning", { alias: ["Självrengörande", "Autosköljning"] }),
  F("Skärm", { alias: ["Display", "Inbyggd display", "Skärmtyp"] }),
  F("App", { alias: ["Smart app", "Appstyrning", "Uppkoppling"] }),
  F("Pumptryck", { enhet: "bar", alias: ["Högtryckspump", "Pump", "Tryck"] }),
  F("Effekt", { enhet: "W", alias: ["Motoreffekt", "Märkeffekt", "Intensitet"] }),
  F("Yttermått", { alias: ["Mått", "Produktmått", "Dimensioner", "Produktstorlek"] }),
  F("Vikt", {
    enhet: "kg",
    alias: ["Nettovikt", "Produktvikt"],
    monster: /(\d+[,.]?\d*)\s*kg\b/i,
 ...
 * ## Wifi-repeater
 *
 * Kategorin har ett fält handeln aldrig anger och som avgör halva köpet:
 * **nätverksuttagets klass**. En repeater med 10/100-uttag mättes av F.A.Z.
 * Kaufkompass till 95 Mbit/s i sladden mot 245 i luften, alltså långsammare på
 * kabel än trådlöst. Sju av tretton har gigabit, två har 2,5 Gbit, två har
 * 10/100 och två saknar uttag helt. Fältet är därför markerat.
 *
 * ⚠️ **`Hastighet 5 GHz` och `Hastighet 2,4 GHz` är två fält och aldrig ett.**
 * Talet på kartongen är summan av båda — AC1900 är 1 300 plus 600 — och en
 * klient sitter på ett band i taget. Ett fält som heter `Hastighet` hade burit
 * marknadsföringens tal och det är precis vad sidan finns för att ta isär.
 * `AC/AX/BE-klass` finns som eget fält för den som söker på kartongens ord.
 *
 * ⚠️ **`Effekt` är tre olika storheter hos tre tillverkare.** TP-Link anger
 * *Max. Power Consumption*, Mercusys *Power Consumption*, D-Link både
 * *Power Consumption* och *Network Standby*, och F.A.Z. **mäter** faktisk
 * drift. Talen är inte jämförbara, så fältet är omarkerat och inget kriterium
 * bär det. `Uppmätt förbrukning` är ett eget fält för de fyra där det finns en
 * oberoende mätning.
 *
 * ⚠️ **`Sändareffekt 2,4 GHz` är omarkerat med flit.** TP-Link anger CE-värdet
 * för sina äldre modeller (RE305 <17 dBm, RE315 ≤20 dBm), men D-Link, Asus och
 * Mercusys anger ingenting och de två Wi-Fi 7-modellerna anger bara
 * FCC-värdet, som gäller i USA. Sex av tretton celler är tomma, vilket är en
 * omarkerad uppgift i produktens egen lista och aldrig en jämförelserad. Se
 * varningen i check:tackning.
 */
const WIFI_REPEATER = [
  F("Pris", { enhet: "kr", alltid: true }),
  F("Wi-Fi-standard", {
    alias: ["Wifi-standard", "Trådlös standard", "Wireless Standards", "Standard", "WLAN-standard"],
    monster: /Wi-?Fi\s*([4-7])\b/i,
  }),
  F("AC/AX/BE-klass", {
    alias: ["Klass", "Hastighetsklass", "Product Segment", "Segment"],
    monster: /\b(A[CX]\d{3,4}|BE\d{3,4})\b/,
  }),
  F("Hastighet 5 GHz", {
    enhet: "Mbit/s",
    alias: ["5 GHz", "Länkhastighet 5 GHz", "Signal Rate 5 GHz", "5GHz"],
    monster: /(\d{3,4})\s*Mbps?\s*(?:at|på|vid)?\s*5\s*GHz/i,
  }),
  F("Hastighet 2,4 GHz", {
    enhet: "Mbit/s",
    alias: ["2,4 GHz", "2.4 GHz", "Länkhastighet 2,4 GHz", "Signal Rate 2.4 GHz", "2.4GHz"],
    monster: /(\d{3,4})\s*Mbps?\s*(?:at|på|vid)?\s*2[,.]4\s*GHz/i,
  }),
  /* Kategorins avgörande fält. Se blockkommentaren ovan. */
  F("Nätverksuttag", {
    alias: ["Ethernet", "Ethernetport", "LAN-port", "Nätverksport", "Interface", "RJ45", "I/O Ports"],
  }),
  F("Mesh", {
    alias: ["Mesh-teknik", "Mesh Technology", "Roaming", "Samma nätverksnamn", "EasyMesh"],
  }),
  F("Antenner", {
    alias: ["Antenn", "Antal antenner", "Antenna", "Antennas"],
    monster: /(\d+)\s*(?:st\.?\s*)?(?:externa|interna|omni)?\s*antenn/i,
  }),
  F("Driftlägen", {
    alias: ["Läge", "Lägen", "Working Modes", "Operating Mode", "Accesspunktläge"],
  }),
  F("Uppmätt förbrukning", {
    enhet: "W",
    alias: ["Uppmätt effekt", "Leistungsaufnahme", "Uppmätt watt"],
  }),
  F("Effekt", {
    enhet: "W",
    alias: ["Effektförbrukning", "Strömförbrukning", "Max. Power Consumption", "Power Consumption"],
    monster: /(\d+[,.]?\d*)\s*W\b/i,
  }),
  F("Sändareffekt 2,4 GHz", {
    enhet: "dBm",
    alias: ["Sändareffekt", "Transmission Power", "Uteffekt"],
    monster: /(?:≤|<)?\s*(\d{2})\s*dBm/i,
  }),
  F("Yttermått", { alias: ["Mått", "Produktmått", "Dimensioner", "Produktstorlek"] }),
  F("Garanti", {
    enhet: "år",
 ...
 * ## Fönsterlarm
 *
 * Det **fristående sirenlarmet**: en batteridriven dosa med magnetkontakt och
 * inbyggd siren som sitter på fönstret och tjuter direkt. Ingen app, ingen
 * hubb, inget konto. Magnetkontakten som i stället rapporterar till en app
 * eller hubb är en annan vara med eget schema, `DORR_OCH_FONSTERSENSOR`.
 *
 * ### `Ljudnivå` är kategorins avgörande fält
 *
 * Spannet är 85 till 130 dB, och eftersom skalan är logaritmisk är det inte en
 * halvering utan en helt annan produkt. Hela Nedis-familjen ligger på 85,
 * eStore på 90, SkyddsExperten på 95 till 100, medan Clas Ohlson och
 * Luxorparts anger 130. Handeln säljer varenda en av dem på ordet "högljudd".
 *
 * ### `Pris per bevakad öppning` är inte styckpriset
 *
 * Fältet bär vad det kostar att larma **ett** fönster, inklusive batterier som
 * inte medföljer. Luxorparts fyrpack kostar 299 kronor men kräver 8 AAA som
 * säljs separat, alltså omkring 92 kronor per fönster. eStore kostar 69 med
 * batterierna i lådan. Räknat per fönster är den dyraste förpackningen inte
 * den dyraste produkten.
 *
 * ### `Avlarmning` är en säkerhetsuppgift och inte en bekvämlighet
 *
 * Ett larm med en på/av-brytare på sidan kan slås av av den som redan tagit
 * sig in. Nedis ALRMD30WT kräver en fyrsiffrig kod på fronten och Luxorparts
 * en fjärrkontroll. Fältet skiljer strömbrytare, kodlås, fjärrkontroll och
 * lägesväljare, och det är den axel som säger mest om vad larmet duger till.
 *
 * ### `Mått larmenhet` saknas för två, och det är inte vår lucka
 *
 * Kjell publicerar inga mått för Luxorparts, manualen är en skannad bild utan
 * textlager, och eStore anger bara vikten. Båda cellerna står tomma.
 * ⚠️ Luxorparts saknade cell gör att omfördelningen i `weightedRating` avgör
 * förstaplatsen. Se blockkommentaren i `lib/data/fonsterlarm.ts`.
 */
const FONSTERLARM = [
  F("Pris", { enhet: "kr", alltid: true }),
  /* Kategorins avgörande fält. Se blockkommentaren ovan. */
  F("Ljudnivå", {
    enhet: "dB",
    alltid: true,
    alias: ["Ljud", "Ljudstyrka", "Sirenstyrka", "Decibel", "Ljudnivå (dB)", "Sound"],
    monster: /(\d+)\s*(?:–|-|till)?\s*(\d+)?\s*dB/i,
  }),
  /* Inte styckpriset. Se blockkommentaren ovan. */
  F("Pris per bevakad öppning", {
    enhet: "kr",
    alltid: true,
    alias: ["Pris per fönster", "Styckpris", "Pris per enhet"],
  }),
  F("Avlarmning", {
    alltid: true,
    alias: ["Aktivering", "Av- och pålarmning", "Aktivering / inaktivering av", "Manövrering"],
  }),
  F("Utlöses av", {
    alltid: true,
    alias: ["Triggad av", "Sensor typ", "Sensortyp", "Detektering"],
  }),
  F("Batterityp", {
    alltid: true,
    alias: ["Batteri", "Kräver batterier", "Batterier", "Strömkälla", "Typ av strömkälla"],
  }),
  F("Batterier medföljer", {
    alias: ["Batteri ingår", "Batterier ingår"],
  }),
  F("Mått larmenhet", {
    enhet: "mm",
    alltid: true,
    alias: ["Mått", "Storlek", "Dimensioner", "Yttermått", "Mått huvudenhet"],
    monster: /(\d+[,.]?\d*)\s*[x×]\s*(\d+[,.]?\d*)(?:\s*[x×]\s*(\d+[,.]?\d*))?\s*mm/i,
  }),
  F("Mått magnet", {
    enhet: "mm",
    alias: ["Magnetens mått", "Storlek magnet", "Magnetkontakt mått"],
  }),
  F("Larmtid", {
    enhet: "s",
    alias: ["Larmlängd", "Larmduration"],
  }),
  F("Antal i förpackningen", {
    alias: ["Antal varor i paketet", "Antal", "Förpackningsantal"],
  }),
  F("Räckvidd fjärrkontroll", {
    enhet: "m",
    alias: ["Räckvidd", "Fjärrkontrollens räckvidd"],
  }),
  F("Vikt", {
    enhet: "g",
    alias: ["Nettovikt", "Produktvikt"],
 ...
 * ## Rörelsevakt utomhus
 *
 * Två lastfält och det är hela poängen. `Last LED` är taket som gäller för den
 * lampa som faktiskt sitter i armaturen, `Last glödljus` är talet som står i
 * annonsen. De skiljer sig med en faktor mellan tre och elva över fältet, och
 * en tabell med bara det ena raderar sidans fynd.
 *
 * ⚠️ `Last LED` uttrycks på fyra sätt av fyra tillverkare, och de är samma
 * begränsning: Steinel räknar mikrofarad och antal don, den nyare
 * IS-generationen watt per lampstorlek, ESYLUX startström i ampere, Kjell och
 * Biltema induktiv respektive lysrörslast. Skriv ut enheten i värdet i stället
 * för att räkna om, för omräkningen finns inte.
 *
 * `Räckvidd` bär villkoret i värdet och inte bara talet. Steinels tolv meter
 * gäller tvärs över synfältet, ESYLUX anger konens djup och bredd var för sig,
 * och Nexas tio meter gäller vid två meters montagehöjd under tjugo grader.
 * Ett naket tal hade gjort tre olika mätningar till en kolumn.
 *
 * `Minsta last` finns eftersom golvet är verkligt och nästan aldrig utskrivet:
 * Steinel IS 240 slår inte till under 10 W.
 */
const RORELSEVAKT_UTOMHUS = [
  F("Pris", { enhet: "kr", alltid: true }),
  F("Typ", { alias: ["Produkttyp"] }),
  F("Bevakningsvinkel", { enhet: "°", alias: ["Detekteringsvinkel", "Avkänningsområde", "Vinkel", "Bevakningsområde"], monster: /(\d{2,3})\s*°/ }),
  F("Räckvidd", { enhet: "m", alias: ["Detekteringsavstånd", "Avkänningsområde räckvidd"], monster: /(\d+[,.]?\d*)\s*(?:m|meter)\b/i }),
  F("Last LED", { alias: ["Maximal belastning LED", "LED-last", "Belastning LED", "Induktiv last"] }),
  F("Last glödljus", { enhet: "W", alias: ["Maxbelastning", "Max belastning", "Max. omkopplingseffekt", "Resistiv last"], monster: /(\d+)\s*W/i }),
  F("Minsta last", { enhet: "W", alias: ["Minsta belastning", "Lägsta belastning"], monster: /(\d+)\s*W/i }),
  F("Kapslingsklass", { alias: ["IP-klass", "Skyddsklass"], monster: /(IP\s?\d{2})/i }),
  F("Drifttemperatur", { enhet: "°C", alias: ["Omgivningstemperatur", "Arbetstemperatur", "Driftstemperatur"], monster: /([+-−]?\d+)\s*(?:–|-|till)\s*([+-−]?\d+)\s*°?\s*C/i }),
  F("Efterlystid", { alias: ["Inkopplingstid", "Lystid", "Tidsintervall", "Efterlysningstid"] }),
  F("Skymningsnivå", { enhet: "lux", alias: ["Ljusnivå", "Luxområde", "Svarsvärde luminositet"], monster: /(\d+)\s*(?:–|-|~|till)\s*(\d+)\s*lux/i }),
  F("Underkrypskydd", { alias: ["Underkrypningsskydd"] }),
  F("Montering", { alias: ["Montage", "Monteringssätt"] }),
  F("Montagehöjd", { enhet: "m", alias: ["Monteringshöjd"] }),
  F("Justering", { alias: ["Vridbar", "Inställbar"] }),
  F("Strömförsörjning", { alias: ["Spänning", "Matning", "Batteri"] }),
  F("Styrning", { alias: ["Protokoll", "Anslutning", "Radio"] }),
  F("Mått", { alias: ["Storlek", "Dimensioner"] }),
  F("Garanti", { enhet: "år" }),
];

export const SPEC_SCHEMA = {
  "rorelsevakt-utomhus": RORELSEVAKT_UTOMHUS,
  "wifi-repeater": WIFI_REPEATER,
  fritos: FRITOS,
  espressomaskin: ESPRESSOMASKIN,
  "dorr-och-fonstersensor": DORR_OCH_FONSTERSENSOR,
  fonsterlarm: FONSTERLARM,
  airfryer: AIRFRYER,
  elscooter: ELSCOOTER,
